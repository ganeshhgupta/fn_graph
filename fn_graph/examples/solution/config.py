...
def load_config(yaml_path: str) -> dict:
    log.debug(f"[config] loading config from {yaml_path}")
    with open(yaml_path, "r") as f:
        config = yaml.safe_load(f)
    run_id = config["pipeline"]["run_id"]
    store_type = config["artifact_store"]["type"]
    nodes = config.get("nodes", {})
    log.info(f"[config] run_id: {run_id}")
    log.debug(f"[config] artifact store type: {store_type}")
    log.debug(f"[config] nodes configured: {list(nodes.keys())}")
    return config


def get_executor(node_config: dict):
    executor_type = node_config.get("executor", "memory")
    log.debug(f"[config] creating executor: {executor_type}")
...
def get_artifact_store(config: dict):
    ...
    run_id = config["pipeline"]["run_id"]
    log.debug(f"[config] creating artifact store: {store_type}")
    if store_type == "fs":
...
def load_stages(config: dict) -> dict:
    ...
    if stages:
        log.info(f"[config] stages defined: {list(stages.keys())}")
        for stage_name, stage_def in stages.items():
            nodes = stage_def.get("nodes", [])
            executor = stage_def.get("executor", "memory")
            log.debug(f"[config]   stage '{stage_name}': executor={executor}, nodes={nodes}")
    else:
        log.info("[config] no stages defined — using node-level execution")
    return stages
